...
def detect_sybil_attack(local_weights, threshold=0.95):
    """
...
def main():
    ...
    # Training with Sybil attacks (Example: Assuming clients 0, 1, 2, 3, 4 are Sybils)
    print("Training with Sybil attacks...")
    train_loss_sybil, train_accuracy_sybil, _ = train_federated(
        args, train_dataset, test_dataset, user_groups, device, logger, malicious_clients=[0, 1, 2, 3, 4]
    )

    # Plot Training Loss vs Communication rounds
    plt.figure()
    plt.title('Training Loss vs Communication Rounds')
    plt.plot(range(len(train_loss_no_sybil)), train_loss_no_sybil, color='r', label='Without Sybil Attacks')
    plt.plot(range(len(train_loss_sybil)), train_loss_sybil, color='b', label='With Sybil Attacks')
    plt.ylabel('Training loss')
    plt.xlabel('Communication Rounds')
    plt.legend()
    loss_plot_path = './save/plots/sybil_training_loss.png'
    ...
    plt.plot(range(len(train_accuracy_no_sybil)), train_accuracy_no_sybil, color='r', label='Without Sybil Attacks')
    plt.plot(range(len(train_accuracy_sybil)), train_accuracy_sybil, color='b', label='With Sybil Attacks')
    plt.ylabel('Average Accuracy')
    plt.xlabel('Communication Rounds')
    plt.legend()
    acc_plot_path = './save/plots/sybil_training_accuracy.png'
